...
func NewContext[B any](w http.ResponseWriter, r *http.Request, options readOptions) *BaseContextWithBody[B] {
	c := &BaseContextWithBody[B]{
...
// BaseContextWithBody is used internally by Fuego. You probably want to use Ctx[B] instead. Please do not use a pointer type as parameter.
type BaseContextWithBody[Body any] struct {
	body *Body // Cache the body in request context, because it is not possible to read an http request body multiple times.
	BaseContext
}

// BaseContext is used when the controller does not have a body.
...
func (c BaseContext) Body() (any, error) {
...
func (c BaseContext) MustBody() any {
	b, err := c.Body()
	if err != nil {
		panic(err)
	}
...
func (c BaseContext) Redirect(code int, url string) (any, error) {
...
func (c BaseContext) Render(templateToExecute string, data any, layoutsGlobs ...string) (HTML, error) {
...
func (c BaseContext) QueryParams() map[string]string {
...
func (c BaseContext) QueryParamIntErr(name string) (int, error) {
...
func (c BaseContext) QueryParamInt(name string, defaultValue int) int {
...
func (c BaseContext) QueryParamBoolErr(name string) (bool, error) {
	param := c.QueryParam(name)
	if param == "" {
...
func (c BaseContext) QueryParamBool(name string, defaultValue bool) bool {
...
func (c BaseContext) MainLang() string {
	return strings.Split(c.MainLocale(), "-")[0]
}

func (c BaseContext) MainLocale() string {
...
// MustBody works like Body, but panics if there is an error.
func (c *BaseContextWithBody[B]) MustBody() B {
	b, err := c.Body()
	if err != nil {
		panic(err)
	}
...
func (c *BaseContextWithBody[B]) Body() (B, error) {
	if c.body != nil {
		return *c.body, nil
	}

	body, err := body[B](c.BaseContext)
	c.body = &body
	return body, err
}

func body[B any](c BaseContext) (B, error) {
